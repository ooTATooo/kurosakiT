...
// 黒崎授業
void KdShaderManager::WriteCBConeLightEnable(bool _enable)
{
	m_cb9_Light.Work().coneLight.enable = _enable;
	// GPUに転送
	m_cb9_Light.Write();
}

void KdShaderManager::WriteCBConeLight(const Math::Vector3& _pos, const Math::Vector3& _dir, const float _angle, const float _range, const Math::Vector3& _color)
{
	cbLight& light = m_cb9_Light.Work();

	light.coneLight.pos = _pos;
	light.coneLight.dir = _dir;
	light.coneLight.angle = _angle;
	light.coneLight.range = _range;
	light.coneLight.color = _color;
	// GPUに転送
	m_cb9_Light.Write();
