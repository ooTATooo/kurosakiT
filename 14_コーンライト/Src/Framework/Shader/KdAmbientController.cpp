...
void KdAmbientController::SetConeLight(const Math::Vector3& _pos, const Math::Vector3& _dir, const float _angle, const float _range, const Math::Vector3& _color)
{
	m_parameter.m_coneLightPos = _pos;
	m_parameter.m_coneLightDir = _dir;
	m_parameter.m_coneLightAngle = _angle;
	m_parameter.m_conrLightRange = _range;
...
void KdAmbientController::WriteLightParams()
{
	...
	if (m_dirtyConeLight)
	{
		KdShaderManager::Instance().WriteCBConeLight(m_parameter.m_coneLightPos, m_parameter.m_coneLightDir, m_parameter.m_coneLightAngle, m_parameter.m_conrLightRange, m_parameter.m_coneLightColor);
