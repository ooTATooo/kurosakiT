...
void Tank::Update()
{
	...
	if (GetAsyncKeyState(VK_LBUTTON) & 0x8000)
	{
...

		{
			...
			Math::Matrix rotMat;
			// 角度を格納する変数(ランダムで-5～5)
			// x,y,zに-5～+5度をラジアン角で格納
			float x = DirectX::XMConvertToRadians(rand() % 11 - 5);
			float y = DirectX::XMConvertToRadians(rand() % 11 - 5);
			float z = DirectX::XMConvertToRadians(rand() % 11 - 5);
			// XYZ方向に回転行列作成
